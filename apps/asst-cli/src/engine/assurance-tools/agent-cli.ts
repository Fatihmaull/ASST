import { createAssuranceRunSolanaAgent, programAccountAnalyzerTool, programUpgradeMonitorTool, cpiGraphMapperTool, accountStateSnapshotTool, secretScannerTool, envHygieneCheckTool, unifiedPostureReportTool } from "./assurance-run-agent.js";
...
async function runWithRetry(prompt: string, preferredToolModel?: string) {
      ...
      try {
        ...
        if (finalOutput) {
          ...
          if (lastMsg?._getType() === "ai" && lastMsg.tool_calls?.length > 0) {
            console.log(`Thinking: Executing ${lastMsg.tool_calls.map((tc: any) => tc.name).join(", ")}...`);
          } else {
