...
const ORCHESTRATOR_SYSTEM_PROMPT = `You are ASST Orchestrator — a Solana security intelligence system.
...
- Pass the repository path and specific context to each agent's task
...
export class Orchestrator {
  ...
  async chat(
    ...
    for (const task of routing) {
      const subAgent = this.subAgents.get(task.agent);
      if (!subAgent) {
        results.push({
          agent: task.agent,
          output: `[Error] Sub-agent "${task.agent}" not found.`
        });
        continue;
      }
      ...
      try {
        ...
        // If this is the report synthesizer, include previous results
        let fullPrompt = taskPrompt;
        if (task.agent === "report_synthesizer" && results.length > 0) {
   ...
   */
  private async route(input: string): Promise<{ agent: string; task: string }[]> {
    try {
      const messages = [
      ...
      if (Array.isArray(parsed)) {
        return parsed.filter(
          (t: any) => typeof t.agent === "string" && typeof t.task === "string"
        );
      }
